...
const Outcome: React.FC<Types> = ({ parentCategories, categoriesByParent }) => {
  ...
  const [isOldRender, setIsOldRender] = React.useState<boolean>(false)
      ...
      {parentCategories.map((item: Category) => (
        <div key={item.uuid}>
          <Link href={`/categories/${item.uuid}`}>
            <div className={`w-full bg-white rounded-lg h-[200px] overflow-hidden hover:drop-shadow p-3 ${(categoriesByParent(item.uuid).length === 0) && 'border-2 border-red-400'}`}>
  ...
  const handleSave = (payload: PopupTypes) => {
    setIsLoading(true)
    ...
    axios
      .patch(`categories/${payload.uuid}/`, { ...validatedPayload.data })
      .then((res) => {
        if (res.status === 200) {
          mutate('categories/')
          toast({
        ...
        {parentCategories.map((item: Category) => (
          ...
          >
            <div className="group flex h-10 w-full justify-start items-center bg-sky-100">
              <AccordionTrigger className="group text-lg">
                {item.name}
              </AccordionTrigger>
              <Popover onOpenChange={(open) => open && setCategoryName(item.name)}>
                ...
                <PopoverContent className="flex flex-col gap-3 w-80 rounded-md bg-white border-none" sideOffset={10}>
                  ...
                  <div className="flex items-end mt-2 justify-between">
                    <div className="flex gap-2">
                      <Button
                        size="sm"
                        disabled={!categoryName || categoryName === item.name}
                        onClick={() => handleSave({...item, name: categoryName || item.name })}
                      >
                        Save
                      </Button>
                    </div>
                    ...
                    { categoriesByParent(item.uuid).map((category: Category) => (
                      <div key={category.uuid}>
                        ...
                        <div className="group flex flex-nowrap my-1 p-1" key={category.uuid}>
                          ...
                          <Popover onOpenChange={(open) => {if (open) {setCategoryName(category.name); setParent(category.parent)}}}>
                            <PopoverTrigger asChild>
                              <span>
                            ...
                            <PopoverContent className="flex flex-col gap-3 w-80 rounded-md bg-white border-none" sideOffset={10}>
                              ...
                              <div className="flex items-end mt-2 justify-between">
                                <div className="flex gap-2">
                                  <Button
                                    size="sm"
                                    disabled={!categoryName || (categoryName === category.name && parent === category.parent)}
                                    onClick={() => handleSave({
                                      ...category,
