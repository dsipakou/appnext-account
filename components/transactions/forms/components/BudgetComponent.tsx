...
const BudgetComponent: React.FC<BudgetComponentTypes> = (params) => {
  ...
  React.useEffect(() => {
    ...
    const activeBudget =
      typeof value === 'string' ? budgets.find((item: WeekBudgetItem) => item.uuid === value) : value;

    apiRef.current.setEditCellValue({
      id,
      field,
      value: row.account.user === activeBudget?.user ? activeBudget : null,
    });
          ...
          <SelectContent>
            ...
            {completedItems.length > 0 && (
              <>
                ...
                <SelectGroup>
                  <SelectLabel>
                    <span className="italic">Completed budgets</span>
                  </SelectLabel>
