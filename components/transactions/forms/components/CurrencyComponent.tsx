...
const CurrencyComponent: React.FC<any> = (params) => {
  ...
  React.useEffect(() => {
    ...
    const isBudgetCurrencyAvailable = availableRates.find(
      (item: AvailableRate) => item.currencyCode === budgetCurrency?.code,
    );
    ...
    const newValue = isBudgetCurrencyAvailable
      ? budgetCurrency
      : isDefaultCurrencyAvailable
        ? defaultCurrency
        : baseCurrency;
    ...
    <div className="flex h-full w-full select-none items-center bg-slate-100">
      {!currencies.length ? (
        ...
        <Select onValueChange={handleChange} value={value}>
          <SelectTrigger className="relative h-full rounded-lg border bg-white">
                ...
                currencies.map((item: Currency) => {
                  ...
                  if (rate) {
                    if (rate.rateDate === transactionDate) {
                      return (
                        <SelectItem key={item.uuid} value={item}>
                          {item.code}
                        </SelectItem>
                      );
                    } else {
                    ...
                    return (
                      <SelectItem key={item.uuid} value={item} disabled>
                        {item.code}
                      </SelectItem>
                    );
                  }
