...
const UnsavedTransactionsAlert: React.FC<{ open: boolean }> = ({ open = false }) => {
  return (
    <Alr.AlertDialog open={open}>
      ...
      <Alr.AlertDialogContent>
        <Alr.AlertDialogHeader>
          ...
          <Alr.AlertDialogDescription>
            Either remove or submit them
          </Alr.AlertDialogDescription>
        </Alr.AlertDialogHeader>
...
const AddForm: React.FC<Types> = ({ open, onOpenChange, url, budget }) => {
  ...
  return (
    <>
      ...
      <UnsavedTransactionsAlert open={alertOpen} />
    </>
  )
}
